...
int logd(long fromAccountNumber, long toAccountNumber, double amount) {
...
int depBalance(long long account_number, double amount) {
    ...
    while (!WindowShouldClose()) {
        ...
        if (CheckCollisionPointRec(mouse, withbutton) && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
            if (amount <= 0) {
                ...
                depBalance(acc_no,amount);
                logd(acc_no,acc_no,+amount);
